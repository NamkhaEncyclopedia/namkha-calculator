...
- **You provide neither, birth before 1880** – the oldest map is drawn for 1880 and says nothing about earlier borders, so no country is looked up. The zone covering the coordinates today is kept. What the clock showed at the birth comes from that *zone's own history*. Almost everywhere that is still the mean solar time of the birth longitude, with the `LOCAL_MEAN_TIME` notice and no caution, because the zone did not provide a concrete offset. A few places had a civil clock this early, London from 1847 among them. There the zone does decide the clock, so the `TIMEZONE_ESTIMATED` caution follows.
...
Each country introduced standard time on its own date, some as late as the 1950s. Until that date, whatever zone applies, the birth runs on the *mean solar time of the birth longitude* and gets the `LOCAL_MEAN_TIME` notice. This overrides a `zone_key` you provide as well, because no zone was in use yet.
...
There is one exception. For the dates where time standardization has not reached the birth region, the offset is read from the birth longitude instead of from the zone. The zone then has no effect on the result, so the caution is left out and the birth carries only the `LOCAL_MEAN_TIME` note.
